...
#else
    RCLCPP_WARN(this->get_logger(), "ZED SDK not available - running in mock mode");
#endif
}
...
void ZedBridge::publishImages()
{
    ...
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000,
                         "publishImages() called in mock mode - no ZED data");
#endif
}
...
void ZedBridge::broadcastTransform()
{
    ...
    this->tf_broadcaster->sendTransform(transform);
#else
    RCLCPP_DEBUG(this->get_logger(), "broadcastTransform skipped in mock mode");
#endif
}
...
#else
    throw std::runtime_error("slMat2cvMat called without ZED SDK available");
#endif
}
